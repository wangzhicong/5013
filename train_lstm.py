...
working_folder = 'C:\\Users\\Wangzhc\\Desktop\\codes\\python code\\5013\\demo3\\'
sys.path.append(working_folder)
from auxiliary import generate_bar, generate_seg
...
def next_batch(start,batch):
    x = []
    y = []
    for i in range(batch):       
        x.append(training_set['x'][start:start+1])
        y.append(training_set['y'][start:start+1])
    return x,y  
...
def preparing_training(filename):
    ...
    keys = list(format2.keys())
    data_load =  len(keys)//bar_length
    data = []
    for i in tqdm(range(data_load)):
        seg = []
        for asset in range(4):
            datas = pd.DataFrame(columns = ['close', 'high', 'low', 'open', 'volume'])
            
            
            
            for j in range(bar_length):
                data_cur_min = format2[keys[i*bar_length+j]][:]
                datas.loc[j] = data_cur_min[asset,]
            
            
            segment = generate_seg(datas) 
            seg +=segment
            
        if len(data) > time_step:
            training_set['x'].append(data[0:time_step].copy())
            training_set['y'].append(data[1:time_step+1].copy())
            data.pop(0)
        else:
            data.append(seg)
...
def train_lstm(save_name,time_step=time_step):
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch*(data_size//batch_size)):
            ...
            x_l,y_l = next_batch((i*batch_size)%(data_size-batch_size-1),batch_size)
            #print(y)
            #print(i,x,y)
            #print(x,y)
