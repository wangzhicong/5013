...
def evaluate_performance(prices, dates, dps, t, step):
    ...
    position_old = 0
    position_new = 0
    total_balance = init_cash

    for i in range(720, len(prices) - 1, step):
        # long position - BUY
        # print(dates[i], ': predicted change of price ', dps[i - 720])
        range_dp = dps[i - 720]
        fee = commissionRatio * prices[i-1]
        # print(i, fee, range_dp)
        if range_dp - fee > 0 and position <= 0:
            range_dp = 0
            position_new += 1
        # short position - SELL
        if (range_dp + fee) < 0 and position >= 0:
        ...
        transaction_cost = np.abs(position_change) * prices[i] * commissionRatio
        revenue = position_old * (prices[i] - prices[i-1]) - transaction_cost
        crypto_balance = np.abs(position_new * prices[i])
        total_balance = total_balance + revenue
        cash_balance = total_balance - crypto_balance
        detail = np.append(dps[i - 720],
            [position_new, cash_balance, crypto_balance, revenue, total_balance, transaction_cost])
        details.append(copy.deepcopy(detail))
        position_old = position_new
        #print(detail)
        if (cash_balance < cash_balance_lower_limit):
            # print(cash_balance)
            print("Current cash balance is lower than", cash_balance_lower_limit)
            print("Your strategy is forced to stop")
            print("System will soon close all your positions (long and short) on crypto currencies")
            position_new = 0
            # break

    detailCol = ['dp', 'positon', "cash_balance", "crypto_balance", "revenue", "total_balance", "transaction_cost"]
    detailsDF = pd.DataFrame(details, index=pd.to_datetime(dates[720:len(dates) - 1]), columns=detailCol)
    detailsDF.to_csv('backtest_details.csv')
    pnl_analyze(detailsDF, 100000)
    return detailsDF
