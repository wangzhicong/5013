...
def generate_bar(data):
    ...
    for i in range(len(data)):
        ...
        #tmp = 
        output.append(100*(sum(data[i][0:4])/sum(data[len(data)//2-1][0:4])-1))#,data[i][4]-data[len(data)//2 -1][4]])
...
def generate_bar2(data):
    output = []
    for i in range(len(data)):
        output.append(100*(data[i][0]/data[len(data)//2 -1][0]-1))#,data[i][1]-data[len(data)//2 -1][1]])
